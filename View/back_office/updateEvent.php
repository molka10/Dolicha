<?php
include 'C:\xampp\htdocs\dolichafinalCopy\controllers\EventC.php'; // Inclure le fichier contenant la classe HotelC
...
if (
    ...
    if (
        ...
        $id = $_POST['id'];
        $currentEvent = $EventC->getEventById($id);
        if (!$currentEvent) {
            $error = "Event non trouvé.";
            exit;
        }
        ...
        if ($_FILES["image"]["size"] != 0) {
            ...
            $imageName = $currentEvent['image']; // Keep the existing image
        }
        ...
        $success = $EventC->updateEvent($id, $nom, $duration, $date, $lieu, $description, $prix, $imageName);
        if ($success) {
