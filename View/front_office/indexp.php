...
function imageExists($url) {
    // Use getimagesize to check if the image exists
    return @getimagesize($url) !== false;
}
?>
        ...
        <div class="row d-flex justify-content-center">
            <div class="menu-content pb-40 col-lg-8">
                <div class="title text-center">
                    <h1 class="mb-10">Artisanat Tunisien</h1>
                    <p>Accessoires uniques</p>
                </div>
            </div>
        </div>
        <div class="row">
            <?php if (!empty($products)): ?>
                <?php foreach ($products as $product): ?>
                    <?php
                    // Assuming $product->getImage() returns the image filename, e.g., 'image.jpg'
                    $imagePath = '/dolicha0.2/View/uploads/' . urlencode(basename($product->getImage()));

                    // Set a default image if the product image is not available or file does not exist
                    if (empty($product->getImage()) || !imageExists($_SERVER['DOCUMENT_ROOT'] . $imagePath)) {
                        $imagePath = '/dolicha0.2/View/uploads/d2.jpg'; // default image
                    }
                    ?>
                    <div class="col-lg-4">
                        <div class="single-destinations">
                            <div class="thumb">
                                <img src="<?= $imagePath ?>" alt="Image of <?= htmlspecialchars($product->getName()) ?>" class="product-image">
                            </div>
                            <div class="details">
                                <h4><?= htmlspecialchars($product->getName()); ?></h4>
                                <p>Découvrez notre produit artisanal unique. Fabriqué avec soin pour répondre à vos besoins.</p>
                                <ul class="package-list">
                                    <li class="d-flex justify-content-between align-items-center">
                                        <span>Prix</span>
                                        <a href="#" class="price-btn"><?= htmlspecialchars($product->getPrice()); ?> DT</a>
                                    </li>
                                    <li class="d-flex justify-content-between align-items-center">
                                        <span>Stock</span>
                                        <span><?= htmlspecialchars($product->getStock()); ?></span>
                                    </li>
